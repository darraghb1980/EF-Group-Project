...
{
    public class ResultsContribParse : AllDataParser
    {
        ...
        List<Contributions> AllDataParser.parseContributions()
        {
            ...
            while (csv.ReadNextRecord())
            {
                ...
                for (int i = 0; i < fieldCount; i++)
                {
                    ...
                    else if (headers[i].Equals("CONTRIBUTIONTYPE"))
                    {
                        exObj.ContributionType = csv[i];
                    }
                    else if (headers[i].Equals("CONTRIBUTIONTYPEDESC"))
                    {
                        exObj.ContributionTypeDesc = csv[i];
                    }
                    else if (headers[i].Equals("CONTRIBUTORTYPE"))
                    {
                        exObj.ContributorType = csv[i];
                    }
